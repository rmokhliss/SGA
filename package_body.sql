...
-- Generated 18-Feb-21 08:51:06 from API_SIF_MOBILE@(DESCRIPTION =(ADDRESS_LIST =(ADDRESS = (PROTOCOL = TCP)(HOST = 172.18.1.199)(PORT = 1528)))(CONNECT_DATA =(SERVICE_NAME = afb12c)))
...
IS
    ...
    IS
        ...
        V_RESREG REGISTRATIONRS;
    BEGIN
        V_RESPONSE := CARDINFORS(NULL, NULL);
        V_RESPONSE.STATUS := STATUSSTRUCT (NULL, NULL, NULL);
        V_RESPONSE.STATUS.REQUESTID := V_REQUEST.INITIATOR.REQUESTID;
        V_RESPONSE.STATUS.ERRORCODE := MB_RESP_PKG.MB_TRACE_C;
        V_RESPONSE.STATUS.ERRORDESC := 'INIT GETCARDINFO';
        ...
        IF RET != MB_INTERFACE_TOOLS_PKG.OK
        THEN
            V_RESPONSE.CARD := NULL;
            V_RESPONSE.STATUS := V_RESREG.STATUS;
            GOTO END_PROCESS;
        END IF;
        --END CHECK REGISTRATION
        RET := MB_INTERFACE_TOOLS_PKG.PROCESSCARDINFO (V_REQUEST,V_RESPONSE);
       ...
       <<END_PROCESS>>
        L_REQUEST_XML := XMLTYPE (V_REQUEST);
        L_RESPONSE_XML := XMLTYPE (V_RESPONSE);
        MB_INTERFACE_TOOLS_PKG.LOG_MB_EXCHANGE ('GETCARDINFO',
                                                NULL,
                                                V_REQUEST.INITIATOR.REQUESTID,
                                                  L_REQUEST_XML,
                                                  L_RESPONSE_XML);
    EXCEPTION
        ...
        THEN
            ROLLBACK;
            V_RESPONSE.STATUS.ERRORCODE := MB_RESP_PKG.MB_SQL_C;
            V_RESPONSE.STATUS.ERRORDESC := 'A40: '||SUBSTR (SQLERRM, 1,74);
            MB_INTERFACE_TOOLS_PKG.TRACE_MB_EXCHANGE (
                $$PLSQL_UNIT,
                'GETCARDINFO',
                $$PLSQL_LINE,
    ...
    BEGIN
        ...
        V_RESPONSE := CREATEDEBITRS(NULL);
        V_RESPONSE.STATUS := STATUSSTRUCT (NULL, NULL, NULL);
        V_RESPONSE.STATUS.REQUESTID := V_REQUEST.INITIATOR.REQUESTID;
        V_RESPONSE.STATUS.ERRORCODE := MB_RESP_PKG.MB_TRACE_C;
        V_RESPONSE.STATUS.ERRORDESC := 'INIT CREATEDEBIT';
        ...
        --END CHECK REGISTRATION
        RET := MB_INTERFACE_TOOLS_PKG.PROCESSCREATEDEBIT (V_REQUEST,V_RESPONSE);
        IF RET != MB_INTERFACE_TOOLS_PKG.OK
        THEN
            GOTO END_PROCESS;
        END IF;
    ...
    EXCEPTION
        ...
        THEN
            ROLLBACK;
            V_RESPONSE.STATUS.ERRORCODE := MB_RESP_PKG.MB_SQL_C;
            V_RESPONSE.STATUS.ERRORDESC := 'CD1: '||SUBSTR (SQLERRM, 1,74);
            MB_INTERFACE_TOOLS_PKG.TRACE_MB_EXCHANGE (
                $$PLSQL_UNIT,
                'CREATEDEBIT',
                $$PLSQL_LINE,
  ...
  PROCEDURE REPLACECARD (
    ...
    BEGIN
        ...
        V_RESREG.STATUS.REQUESTID := V_REQUEST.INITIATOR.REQUESTID;
        RET := MB_INTERFACE_TOOLS_PKG.WALLETREGISTRATIONCONTROL (V_REQREG,V_RESREG);
        IF ((RET != MB_INTERFACE_TOOLS_PKG.OK) AND (RET!=MB_INTERFACE_TOOLS_PKG.CARD_NOT_ACTIVE))
        THEN
    ...
    BEGIN
        ...
        V_RESPONSE := ENROLLRS(NULL);
        V_RESPONSE.STATUS := STATUSSTRUCT (NULL, NULL, NULL);
        V_RESPONSE.STATUS.REQUESTID := V_REQUEST.INITIATOR.REQUESTID;
        V_RESPONSE.STATUS.ERRORCODE := MB_RESP_PKG.MB_TRACE_C;
        V_RESPONSE.STATUS.ERRORDESC := 'INIT ENROLL';

        RET := MB_INTERFACE_TOOLS_PKG.PROCESSENROLLMENT (V_REQUEST,V_RESPONSE);
       ...
       <<END_PROCESS>>
        L_REQUEST_XML := XMLTYPE (V_REQUEST);
        L_RESPONSE_XML := XMLTYPE (V_RESPONSE);
        MB_INTERFACE_TOOLS_PKG.LOG_MB_EXCHANGE ('ENROLL',
                                                NULL,
                                                V_REQUEST.INITIATOR.REQUESTID,
                                                  L_REQUEST_XML,
                                                  L_RESPONSE_XML);
...
-- Generated 18-Feb-21 08:51:07 from API_SIF_MOBILE@(DESCRIPTION =(ADDRESS_LIST =(ADDRESS = (PROTOCOL = TCP)(HOST = 172.18.1.199)(PORT = 1528)))(CONNECT_DATA =(SERVICE_NAME = afb12c)))
...
IS
   ...
   FUNCTION WALLETGETREGISTRATION (
    ...
    BEGIN
        P_STATUS := STATUSSTRUCT (NULL, NULL, NULL);
        P_STATUS.REQUESTID := V_RESPONSE.STATUS.REQUESTID;
        P_STATUS.ERRORCODE := MB_RESP_PKG.MB_90016_C;
        P_STATUS.ERRORDESC := MB_RESP_PKG.MB_90016_D;

        RET := CHECKCUSTOMER(V_REQUEST.CLIENTID, P_CUS_CODE, P_STATUS);

        IF(RET = CUSTOMER_ENROLLED) THEN--case

             RET := CHECKACCOUNT(V_REQUEST.ACOUNTNBR, P_CUS_CODE, P_STATUS);
               IF RET IN ( ACCOUNT_NOT_CORRECT, ACCOUNT_NOT_MAD, ACCOUNT_NOT_ACTIVE) THEN--account exist but not valid
                     RET := ERROR;
               END IF;

        ELSIF(RET = CUSTOMER_NOT_ACTIVE) THEN
                RET := ERROR;--exist but not active
        END IF;
    ...
    EXCEPTION
        ...
        THEN
            ROLLBACK;
            V_RESPONSE.STATUS.ERRORCODE := MB_RESP_PKG.MB_SQL_C;
            V_RESPONSE.STATUS.ERRORDESC := 'En0: '||SUBSTR (SQLERRM, 1,74); MB_INTERFACE_TOOLS_PKG.TRACE_MB_EXCHANGE (
                $$PLSQL_UNIT,
                'WALLETGETREGISTRATION',
                $$PLSQL_LINE,
                V_RESPONSE.STATUS.ERRORDESC);
            RETURN ERROR;
    END WALLETGETREGISTRATION;
   ...
   /*  V_RESPONSE.CARDS.EXTEND;
    ...
    IS
      ...
      CURSOR CARDS_CUR (
            V_CUS_IDEN    CUSTOMER_CARDHOLDER.CUS_IDEN%TYPE, V_CARD_CODE    CUSTOMER_CARDHOLDER.CUS_IDEN%TYPE)
        IS
            SELECT  CAR_CODE AS CARDCODE, CAR_NUMB, NVL (MASK_PAN ('Y', CAR_NUMB), '0') AS PCIPAN,
                    SUBSTR(NVL (TRIM(CAR_CHLD_NAME), CAR_CHLD_FIRS_NAME),1,25)  AS NAMEPRINTED,
                    NVL(CAR_REPL_NEW_EXPI_DATE, CAR_EXPI_DATE) AS EXPIRYDATE,
   ...
   /*  V_RESPONSE.CARDS.EXTEND;
     V_RESPONSE.CARD :=CARDSTRUCT (12456, '4256XXXXXXXX1145','HASSAN NOURI','12','mastercard',SYSDATE,2000,1,0);*/

        FOR CARDS_ROW IN CARDS_CUR (V_REQUEST.INITIATOR.CLIENTID, V_REQUEST.INITIATOR.CARDCODE)
        LOOP
            ...
            V_RESPONSE.CARD :=
                CARDSTRUCT (NULL,NULL,
                                ...
                                NULL,NULL,
                                1);
            V_RESPONSE.CARD.CARDCODE := CARDS_ROW.CARDCODE;
            V_RESPONSE.CARD.PCIPAN := CARDS_ROW.PCIPAN;
            V_RESPONSE.CARD.NAMEPRINTED := CARDS_ROW.NAMEPRINTED;
            V_RESPONSE.CARD.EXPIRYDATE := CARDS_ROW.EXPIRYDATE;
            V_RESPONSE.CARD.PRODUCTCODE := CARDS_ROW.CAR_CPR_CODE;
            V_RESPONSE.CARD.PRODUCTDESC := CARDS_ROW.CPR_LABE;
            V_RESPONSE.CARD.LOYALTYBALANCE := NULL;
            V_RESPONSE.CARD.ACTIVATION := CARDS_ROW.CARDSTATUS;
    ...
    EXCEPTION
        ...
        THEN
            ...
            V_RESPONSE.CARD := NULL;
            V_RESPONSE.STATUS.ERRORCODE := MB_RESP_PKG.MB_SQL_C;
            V_RESPONSE.STATUS.ERRORDESC := 'P40: '||SUBSTR (SQLERRM, 1,74);
            MB_INTERFACE_TOOLS_PKG.TRACE_MB_EXCHANGE (
                $$PLSQL_UNIT,
                'PROCESSCARDINFO',
                $$PLSQL_LINE,
            ...
            IF TRANS_ROW.AUT_RESP_CODE_F039 = '000' THEN
             ...
             IF TRANS_ROW.AUT_REVE_STAT = 'N' THEN
                IF TRANS_ROW.AUT_VALI_STAT = 'VALI' THEN
                    V_RESPONSE.TRANSACTIONS (V_COUNT).STATUS := 1;--VALIDÉE
                ELSE
                    V_RESPONSE.TRANSACTIONS (V_COUNT).STATUS := 0;--EN  ATT VALIDATION
                END IF;
             ELSE
                V_RESPONSE.TRANSACTIONS (V_COUNT).STATUS := 2;--ANNULÉE
             END IF;
            ELSE
                V_RESPONSE.TRANSACTIONS (V_COUNT).STATUS := 3;--REJETÉE
...
*/
    ...
    IS
        RET               NUMBER;
        P_DEBIT_APPLICATION   DEBIT_APPLICATION%ROWTYPE;
        P_DAP_CODE DEBIT_APPLICATION.DAP_CODE %TYPE;
   BEGIN


         BEGIN
          SELECT *
          INTO  P_DEBIT_APPLICATION
          FROM  DEBIT_APPLICATION
          WHERE   DAP_CUS_IDEN  = V_REQUEST.INITIATOR.CLIENTID;
         EXCEPTION
          WHEN OTHERS THEN
               P_DEBIT_APPLICATION := NULL ;
         END ;


       SELECT XDEBIT_APPLICATION.NEXTVAL INTO  P_DAP_CODE  FROM DUAL ;
       P_DEBIT_APPLICATION.DAP_CODE         := P_DAP_CODE ;
       P_DEBIT_APPLICATION.DAP_BAN_CODE     := 1;
       P_DEBIT_APPLICATION.DAP_IDEN         :=  'MOB'||P_DAP_CODE ;
       P_DEBIT_APPLICATION.DAP_DATE         :=  sysdate ;
       P_DEBIT_APPLICATION.DAP_CUS_IDEN     :=  V_REQUEST.INITIATOR.CLIENTID ;
       P_DEBIT_APPLICATION.DAP_CHLD_NAME    :=   V_REQUEST.DEBITAPP.NAMEPRINTED  ;
       P_DEBIT_APPLICATION.DAP_REQU_ACC_NUMB:= V_REQUEST.INITIATOR.ACOUNTNBR;
       P_DEBIT_APPLICATION.DAP_CHLD_PHONE   := V_REQUEST.DEBITAPP.PHONENUM;

       INSERT_TRACKING (270955,'P_DAP_CODE: '||P_DAP_CODE,0,0);


       BEGIN
           SELECT  CPR_CODE, CPR_PFE_CODE  , CPR_CMF_CODE, CPR_IRP_CODE
           INTO    P_DEBIT_APPLICATION.DAP_REQU_CPR_CODE,
                   P_DEBIT_APPLICATION.DAP_PERS_FEE_CODE,P_DEBIT_APPLICATION.DAP_MEMB_FEE_CODE,
                   P_DEBIT_APPLICATION.DAP_IRP_CODE
           FROM    CARD_PROGRAM
              WHERE CPR_CODE = V_REQUEST.DEBITAPP.PRODUCTCODE;
         EXCEPTION
          WHEN OTHERS THEN
                    V_RESPONSE.STATUS.ERRORCODE := MB_RESP_PKG.MB_90003_C;
                    V_RESPONSE.STATUS.ERRORDESC := MB_RESP_PKG.MB_90003_D;
       ...
       END;
        ...
        BEGIN
           SELECT  BRA_CODE
           INTO    P_DEBIT_APPLICATION.DAP_BRA_CODE
           FROM    BRANCH
              WHERE BRA_IDEN = V_REQUEST.DEBITAPP.IDENAGENCE;
        EXCEPTION
          WHEN OTHERS THEN
                    V_RESPONSE.STATUS.ERRORCODE := MB_RESP_PKG.MB_90017_C;
                    V_RESPONSE.STATUS.ERRORDESC := MB_RESP_PKG.MB_90017_D;
                    RETURN ERROR;
        END ;

         /*   V_REQUEST.DEBITAPP.PAIEMENTSTATUS;
          */

        P_DEBIT_APPLICATION.DAP_FIRS_NAM1  :=  V_REQUEST.DEBITAPP.NAMEPRINTED;
/*

       P_DEBIT_APPLICATION.DAP_GEND1      :=  CUSTOMER.GENDRE;
       P_DEBIT_APPLICATION.DAP_TIT1       :=  CUSTOMER.TITEL;
       P_DEBIT_APPLICATION.DAP_LAST_NAM1  :=  CUSTOMER.LASTNAME;
       P_DEBIT_APPLICATION.DAP_FIRS_NAM1  :=  CUSTOMER.FIRSTNAME;
       P_DEBIT_APPLICATION.DAP_MIDD_NAM1  :=  CUSTOMER.FIRSTNAME;
       P_DEBIT_APPLICATION.DAP_BIRT_DAT1  :=  CUSTOMER.DATEOB;
       P_DEBIT_APPLICATION.DAP_BIRT_LOC1  := '';
       P_DEBIT_APPLICATION.DAP_FIRS_IDEN_TYP1 :=  CUSTOMER.IDENTYPE;
       P_DEBIT_APPLICATION.DAP_FIRS_IDE1  :=  CUSTOMER.IDENVALUE;
       P_DEBIT_APPLICATION.DAP_MST_COD1   := NULL;
       P_DEBIT_APPLICATION.DAP_COU_COD1   :=  P_COU_CODE ;
        P_DEBIT_APPLICATION.DAP_DEPE_NUM1  :=  NULL;
       P_DEBIT_APPLICATION.DAP_PPO_COD1   :=  NULL;
       P_DEBIT_APPLICATION.DAP_CHLD_CORP_NAME   :=   null ;*/
       P_DEBIT_APPLICATION.DAP_CHLD_TITL :=P_DEBIT_APPLICATION.DAP_TIT1   ;
       P_DEBIT_APPLICATION.DAP_CHLD_FIRS_NAME :=P_DEBIT_APPLICATION.DAP_FIRS_NAM1 ;
       P_DEBIT_APPLICATION.DAP_CHLD_MIDD_NAME :=P_DEBIT_APPLICATION.DAP_MIDD_NAM1 ;
       P_DEBIT_APPLICATION.DAP_CHLD_LAST_NAME :=P_DEBIT_APPLICATION.DAP_LAST_NAM1 ;
       P_DEBIT_APPLICATION.DAP_OLD_NEW_ACCT  := 'O'  ;
        P_DEBIT_APPLICATION.DAP_CUS_TYPE     :=  'O' ;

         INSERT INTO DEBIT_APPLICATION VALUES P_DEBIT_APPLICATION ;
    ...
    EXCEPTION
        ...
        THEN
            ROLLBACK;
            V_RESPONSE.STATUS.ERRORCODE := MB_RESP_PKG.MB_SQL_C;
            V_RESPONSE.STATUS.ERRORDESC := SUBSTR (SQLERRM, 1,70);
            MB_INTERFACE_TOOLS_PKG.TRACE_MB_EXCHANGE (
                $$PLSQL_UNIT,
                'PROCESSCREATEDEBIT',
                $$PLSQL_LINE,
...
*/
   ...
   FUNCTION PROCESSENROLLMENT (
    ...
    IS
        RET               NUMBER;
        RET_STT           NUMBER;
        V_CUSTOMER_TABLE_REC          CUSTOMER_CARDHOLDER%ROWTYPE;
        V_CARD_TABLE_REC              CARD%ROWTYPE;
        V_ADRESS_TABLE_REC            ADDRESS%ROWTYPE;
        V_ACCOUNT_TABLE_REC           ACCOUNT%ROWTYPE;
        V_CARD_PROGRAM_TABLE_REC      CARD_PROGRAM%ROWTYPE;
        V_NATIONALBIN_TABLE_REC       NATIONAL_BIN%ROWTYPE;
        RESPONSE_CODE                 NUMBER;
        MYACC_IAF_APLY                ACCOUNT.ACC_IAF_APLY%TYPE;
        SORTIE NUMBER:=1;
        CARSEQ NUMBER :=1;
        V_CAR_NUMB                    CARD.CAR_NUMB%TYPE;
        V_MOBILE                      VARCHAR2 (20);
        V_CITY_CODE                   NUMBER(10,0);
        V_COUNTER                     NUMBER;
        P_IBR_CODE                    NUMBER;
        V_ROUTING_TABLE_REC           CARDHOLDER_ROUTING%ROWTYPE;
        P_CCO_CODE                    NUMBER;
        PP_CCO_CODE                   NUMBER;--TEST
        P_MOBILE V_ADRESS_TABLE_REC.ADR_FIRS_PHON%TYPE;
        P_ACC_NUMB V_ACCOUNT_TABLE_REC.ACC_NUMB%TYPE;
        P_CUS_IDEN V_CUSTOMER_TABLE_REC.CUS_IDEN%TYPE;
        V_CUS_CSR_CODE V_CUSTOMER_TABLE_REC.CUS_CSR_CODE%TYPE;
        V_REQREG REGISTRATIONRQ;
        V_RESREG REGISTRATIONRS;
        V_COUNT         NUMBER:=0;
        V_TRACE VARCHAR(10);
   ...
   BEGIN
        ...
        V_CUSTOMER_TABLE_REC := NULL;
        V_TRACE := 'START ';
        P_MOBILE := V_REQUEST.CUSTOMER.MOBILE;
        P_ACC_NUMB := V_REQUEST.INITIATOR.ACOUNTNBR;
        P_CUS_IDEN := V_REQUEST.INITIATOR.CLIENTID;
        ...
        V_RESREG.STATUS := STATUSSTRUCT (NULL, NULL, NULL);
        V_RESREG.STATUS.REQUESTID := V_REQUEST.INITIATOR.REQUESTID;
        RET_STT := MB_INTERFACE_TOOLS_PKG.WALLETGETREGISTRATION (V_REQREG,V_RESREG);
        IF RET_STT = ERROR
        THEN
            V_RESPONSE.STATUS := V_RESREG.STATUS;
            RETURN RET_STT;
        ...
        --END CHECK REGISTRATION
        V_TRACE := 'REGISTER ';
       IF ( RET_STT = CUSTOMER_NOT_EXIST) THEN
            INCREMENTE_CODE ('CUSTOMER_CARDHOLDER', 'CUS_CODE', V_CUSTOMER_TABLE_REC.CUS_CODE);
            INSERT_TRACKING (270955,'V_CUSTOMER_TABLE_REC.CUS_CODE: '||V_CUSTOMER_TABLE_REC.CUS_CODE,0,0);
            V_CUSTOMER_TABLE_REC.CUS_IDEN := P_CUS_IDEN;
            V_CUSTOMER_TABLE_REC.CUS_TIT1 := V_REQUEST.CUSTOMER.TITEL;
            V_CUSTOMER_TABLE_REC.CUS_FIRS_NAM1 := V_REQUEST.CUSTOMER.FIRSTNAME;
            V_CUSTOMER_TABLE_REC.CUS_LAST_NAM1 := V_REQUEST.CUSTOMER.LASTNAME;
            V_CUSTOMER_TABLE_REC.CUS_BIRT_DAT1 := V_REQUEST.CUSTOMER.DATEOB;
            V_CUSTOMER_TABLE_REC.CUS_FIRS_IDE1 := V_REQUEST.CUSTOMER.IDENVALUE;
            V_CUSTOMER_TABLE_REC.CUS_FIRS_IDEN_TYP1 := V_REQUEST.CUSTOMER.IDENTYPE;          -- CIVIL ID
            V_CUSTOMER_TABLE_REC.CUS_GEND1 := V_REQUEST.CUSTOMER.GENDRE;
            V_CUSTOMER_TABLE_REC.CUS_PRE_PAID_FLAG := 'N'; --CUS_PRE_PAID_FLAG;
            V_CUSTOMER_TABLE_REC.CUS_TYPE := 'I';
            V_CUSTOMER_TABLE_REC.CUS_BAN_CODE :=1;
            V_CUSTOMER_TABLE_REC.CUS_VCA_CODE := 0;           -- CLIENT NORMAL
            V_CUSTOMER_TABLE_REC.CUS_CCS_CODE := 1;                    --ACTIF
            V_CUSTOMER_TABLE_REC.CUS_CSR_CODE := 0;     -- WALLET REGISTRATION
            V_CUS_CSR_CODE := 1;
            V_CUSTOMER_TABLE_REC.CUS_CREA_DATE:=SYSDATE;


            BEGIN
                SELECT BRA_CODE
                INTO V_CUSTOMER_TABLE_REC.CUS_BRA_CODE
                FROM BRANCH
                WHERE BRA_IDEN = V_REQUEST.CUSTOMER.BRANCHIDEN;
                EXCEPTION
                        WHEN OTHERS
                        THEN
                        V_CUSTOMER_TABLE_REC.CUS_BRA_CODE  := NULL;
            END;

            IF(V_CUSTOMER_TABLE_REC.CUS_BRA_CODE=NULL)
            THEN
                V_RESPONSE.STATUS.ERRORCODE := MB_RESP_PKG.MB_90017_C;
                V_RESPONSE.STATUS.ERRORDESC := MB_RESP_PKG.MB_90017_D;
                RETURN ERROR;
            ...
            BEGIN
                ...
                INTO V_CITY_CODE
                FROM CITY
                WHERE CIT_IDEN = V_REQUEST.CUSTOMER.CITYIDEN;
                EXCEPTION
                        WHEN OTHERS
                        THEN
                        V_CITY_CODE := NULL;
            ...
            IF(V_CITY_CODE=NULL)
            THEN
                V_RESPONSE.STATUS.ERRORCODE := MB_RESP_PKG.MB_90014_C;
                V_RESPONSE.STATUS.ERRORDESC := MB_RESP_PKG.MB_90014_D;
                RETURN ERROR;
            END IF;

            V_TRACE := 'CARDHOLDER';
            INSERT INTO CUSTOMER_CARDHOLDER (CUS_CODE,
                                                 CUS_IDEN,
                                                 CUS_TIT1,
                                                 CUS_FIRS_NAM1,
                                                 CUS_LAST_NAM1,
                                                 ...
                                                 CUS_VCA_CODE,
                                                 CUS_CCS_CODE,
                                                 CUS_CSR_CODE,CUS_COU_COD1,CUS_CREA_DATE
                                                 )
                VALUES (V_CUSTOMER_TABLE_REC.CUS_CODE,
                        V_CUSTOMER_TABLE_REC.CUS_IDEN,
                        V_CUSTOMER_TABLE_REC.CUS_TIT1,
                        V_CUSTOMER_TABLE_REC.CUS_FIRS_NAM1,
                        V_CUSTOMER_TABLE_REC.CUS_LAST_NAM1,
                        V_CUSTOMER_TABLE_REC.CUS_BIRT_DAT1,
                        V_CUSTOMER_TABLE_REC.CUS_FIRS_IDE1,
                        V_CUSTOMER_TABLE_REC.CUS_FIRS_IDEN_TYP1,
                        V_CUSTOMER_TABLE_REC.CUS_GEND1,
                        V_CUSTOMER_TABLE_REC.CUS_PRE_PAID_FLAG,
                        V_CUSTOMER_TABLE_REC.CUS_TYPE,
                        V_CUSTOMER_TABLE_REC.CUS_BRA_CODE,
                        V_CUSTOMER_TABLE_REC.CUS_BAN_CODE,
                        V_CUSTOMER_TABLE_REC.CUS_VCA_CODE,
                        V_CUSTOMER_TABLE_REC.CUS_CCS_CODE,
                        V_CUSTOMER_TABLE_REC.CUS_CSR_CODE,   -- WALLET REGISTRATION
                        V_CUSTOMER_TABLE_REC.CUS_COU_COD1  ,
                        V_CUSTOMER_TABLE_REC.CUS_CREA_DATE);
                    INSERT_TRACKING (270955,'CUSTOMER_CARDHOLDER OK',0,0);
            ELSE--CUSTOMER ALREADY EXIST
                 V_CUS_CSR_CODE := 2;
                 V_TRACE := 'READ CHD ';
                 BEGIN
                    SELECT *
                      INTO V_CUSTOMER_TABLE_REC
                      FROM CUSTOMER_CARDHOLDER
                     WHERE CUS_IDEN=V_REQUEST.INITIATOR.CLIENTID;
                 END;
            END IF;

            IF ( RET_STT = ACCOUNT_CORRECT) THEN
                V_TRACE := 'READ ACC ';
                BEGIN
                   SELECT *
                     INTO V_ACCOUNT_TABLE_REC
                     FROM ACCOUNT
                    WHERE ACC_NUMB_MXP=V_REQUEST.INITIATOR.ACOUNTNBR
                     AND  ROWNUM = 1;
                END;
            ELSE --ACCOUNT NOT EXIST
                V_ACCOUNT_TABLE_REC := NULL;
               /* V_TRACE := 'GENERATION';
                RET :=ACCOUNT_GENERATION_PKG.ACCOUNT_NUMBER_GENERATION (
                        V_CUSTOMER_TABLE_REC.CUS_BAN_CODE,
                        'BIDON',
                        V_ACCOUNT_TABLE_REC.ACC_NUMB_MXP,
                        RESPONSE_CODE);

                IF RET != 0
                THEN
                    V_RESPONSE.STATUS.ERRORCODE := MB_RESP_PKG.MB_90012_C;
                    V_RESPONSE.STATUS.ERRORDESC := 'ERROR ACCOUNT_NUMBER_GENERATION RESPONSE_CODE: '||RESPONSE_CODE;
                    MB_INTERFACE_TOOLS_PKG.TRACE_MB_EXCHANGE (
                        $$PLSQL_UNIT,
                        'PROCESSENROLLMENT',
                        $$PLSQL_LINE,
                ...
                INSERT_TRACKING (270955,'V_ACCOUNT_TABLE_REC.ACC_NUMB_MXP: '||V_ACCOUNT_TABLE_REC.ACC_NUMB_MXP,0,0);*/

                V_TRACE := 'ACC_CODE';
                SELECT MAX(ACC_CODE)+1 INTO V_ACCOUNT_TABLE_REC.ACC_CODE FROM ACCOUNT;
               --  INCREMENTE_CODE ('ACCOUNT', 'ACC_CODE', V_ACCOUNT_TABLE_REC.ACC_CODE);

                INSERT_TRACKING (270955,'V_ACCOUNT_TABLE_REC.ACC_CODE: '||V_ACCOUNT_TABLE_REC.ACC_CODE,0,0);
                V_ACCOUNT_TABLE_REC.ACC_NUMB := P_ACC_NUMB ;
                V_ACCOUNT_TABLE_REC.ACC_NUMB_MXP := P_ACC_NUMB ;
                V_ACCOUNT_TABLE_REC.ACC_CURR_CODE := 37;
                V_ACCOUNT_TABLE_REC.ACC_ATY_CODE := 1;
                V_ACCOUNT_TABLE_REC.ACC_CREA_DATE := SYSDATE;
                V_ACCOUNT_TABLE_REC.ACC_EFFE_DATE := SYSDATE;
                V_ACCOUNT_TABLE_REC.ACC_AST_CODE := 1;
                V_ACCOUNT_TABLE_REC.ACC_CUS_CODE := V_CUSTOMER_TABLE_REC.CUS_CODE;
                V_ACCOUNT_TABLE_REC.ACC_TYPE := 'CARDHOLDER';
                V_ACCOUNT_TABLE_REC.ACC_IAF_APLY := MYACC_IAF_APLY;
                V_ACCOUNT_TABLE_REC.ACC_BAN_CODE := V_CUSTOMER_TABLE_REC.CUS_BAN_CODE;
                V_ACCOUNT_TABLE_REC.ACC_BRA_CODE := V_CUSTOMER_TABLE_REC.CUS_BRA_CODE;
                V_TRACE := 'ACCOUNT';
                 INSERT INTO ACCOUNT (ACC_CODE,
                                                 ACC_NUMB_MXP,
                                                 ACC_NUMB,
                                                 ACC_CURR_CODE,
                                                 ACC_ATY_CODE,
                                                 ...
                                                 ACC_AVAI_AMOU,
                                                 ACC_PPR_CODE,
                                                 ACC_INAC_ACCO_FEES_MONT,
                                                 ACC_INAC_ACCO_FEES_RATE,
                                                 ACC_INAC_ACCO_FEES_FIXE,
                                                 ACC_INAC_ACCO_FEES_MIN,
                                                 ACC_INAC_ACCO_FEES_MAX,
                                                 ACC_BREA_AGE,
                                                 ACC_RELO_MAX_LIMI_ON_ACCO,
                                                 ACC_RELO_MAX_RELO_VALU,
                                                 ACC_RELO_MIN_RELO_VALU,
                                                 ACC_AST_CODE,
                                                 ACC_CUS_CODE,
                                                 ACC_TYPE,
                                                 ACC_IAF_APLY,
                                                 ...
                                                 ACC_BRA_CODE,
                                                 ACC_USED_CUMU)
                            VALUES (V_ACCOUNT_TABLE_REC.ACC_CODE,
                                    V_ACCOUNT_TABLE_REC.ACC_NUMB_MXP,
                                    V_ACCOUNT_TABLE_REC.ACC_NUMB,
                                    V_ACCOUNT_TABLE_REC.ACC_CURR_CODE,
                                    V_ACCOUNT_TABLE_REC.ACC_ATY_CODE,
                                    V_ACCOUNT_TABLE_REC.ACC_CREA_DATE,
                                    V_ACCOUNT_TABLE_REC.ACC_EFFE_DATE,
                                    V_ACCOUNT_TABLE_REC.ACC_CLOS_DATE,
                                    V_ACCOUNT_TABLE_REC.ACC_AVAI_AMOU,
                                    V_ACCOUNT_TABLE_REC.ACC_PPR_CODE,
                                    V_ACCOUNT_TABLE_REC.ACC_INAC_ACCO_FEES_MONT,
                                    V_ACCOUNT_TABLE_REC.ACC_INAC_ACCO_FEES_RATE,
                                    V_ACCOUNT_TABLE_REC.ACC_INAC_ACCO_FEES_FIXE,
                                    V_ACCOUNT_TABLE_REC.ACC_INAC_ACCO_FEES_MIN,
                                    V_ACCOUNT_TABLE_REC.ACC_INAC_ACCO_FEES_MAX,
                                    V_ACCOUNT_TABLE_REC.ACC_BREA_AGE,
                                    V_ACCOUNT_TABLE_REC.ACC_RELO_MAX_LIMI_ON_ACCO,
                                    V_ACCOUNT_TABLE_REC.ACC_RELO_MAX_RELO_VALU,
                                    V_ACCOUNT_TABLE_REC.ACC_RELO_MIN_RELO_VALU,
                                    V_ACCOUNT_TABLE_REC.ACC_AST_CODE,
                                    V_ACCOUNT_TABLE_REC.ACC_CUS_CODE,
                                    V_ACCOUNT_TABLE_REC.ACC_TYPE,
                                    V_ACCOUNT_TABLE_REC.ACC_IAF_APLY,
                                    V_ACCOUNT_TABLE_REC.ACC_BAN_CODE,
                                    V_ACCOUNT_TABLE_REC.ACC_BRA_CODE,
                                    V_ACCOUNT_TABLE_REC.ACC_USED_CUMU);

                INSERT_TRACKING (270955,'ACCOUNT OK',0,0);
            END IF;
            ...
            V_RESPONSE.STATUS.REQUESTID := V_REQUEST.INITIATOR.REQUESTID;
            V_RESPONSE.STATUS.ERRORCODE := MB_RESP_PKG.MB_TRACE_C;
            V_RESPONSE.STATUS.ERRORDESC := 'INIT PROCESSVIRTUELCARD';

          /*  RET := PROCESSVIRTUELCARD(V_CUSTOMER_TABLE_REC, V_ACCOUNT_TABLE_REC, V_REQUEST.CUSTOMER.ADDR1, V_REQUEST.CUSTOMER.ADDR2, P_MOBILE,
          V_REQUEST.CUSTOMER.EMAIL, V_CITY_CODE, V_CUS_CSR_CODE, V_RESPONSE);*/
    ...
    EXCEPTION
        ...
        THEN
            ...
            MB_INTERFACE_TOOLS_PKG.TRACE_MB_EXCHANGE (
                ...
                V_RESPONSE.STATUS.ERRORDESC);
            RETURN ERROR;
    END PROCESSENROLLMENT;

    FUNCTION PREPARE_APPLICATION( CUSTOMER     IN OUT NOCOPY  CUSTOMERSTRUCT ,
                                    CUS_IDEN IN VARCHAR2,
                                  P_DEBIT_APPLICATION  OUT DEBIT_APPLICATION%ROWTYPE)
    RETURN NUMBER
    IS

        REJI_MESSAGE          EXCEPTION ;
        P_REJI_CODE           VARCHAR2(40);
        REJI_MESS             VARCHAR2(4):='FALS';
        RESPONSE_CODE         VARCHAR2(5);
        PROC_CODE             VARCHAR2(6);
        FUNCTION_CODE         VARCHAR2(3);
        P_COU_CODE            NUMBER(10) ;
        RAISE_FUNC            EXCEPTION ;
        P_CUR_CODE                         NUMBER ;
        CPR_LIFE_CYCL       NUMBER(10) ;
        P_NBI_CODE  NUMBER(10);
        P_NBI_IDEN  NUMBER(11);
        P_PAN_LENG  NUMBER(10);
        RET         NUMBER(10);
        P_DAP_CODE DEBIT_APPLICATION.DAP_CODE %TYPE;
        P_DAP_MEMB_FEE_CODE   NUMBER(10) ;
    BEGIN

       P_DEBIT_APPLICATION := NULL ;
       SELECT XDEBIT_APPLICATION.NEXTVAL INTO  P_DAP_CODE  FROM DUAL ;
       P_DEBIT_APPLICATION.DAP_CODE := P_DAP_CODE ;
       P_DEBIT_APPLICATION.DAP_BAN_CODE := CUSTOMER.BANKCODE;
       P_DEBIT_APPLICATION.DAP_IDEN       :=  'MOB'||P_DAP_CODE ;
       P_DEBIT_APPLICATION.DAP_DATE       :=  sysdate ;
       P_DEBIT_APPLICATION.DAP_CUS_TYPE  :=  'O' ;
       P_DEBIT_APPLICATION.DAP_CUS_IDEN     :=  CUS_IDEN ;

         BEGIN
          SELECT CUS_CODE
          INTO  P_DEBIT_APPLICATION.DAP_CUS_CODE
          FROM  CUSTOMER_CARDHOLDER
          WHERE   CUS_IDEN  = P_DEBIT_APPLICATION.DAP_CUS_IDEN
             AND  CUS_BAN_CODE  = P_DEBIT_APPLICATION.DAP_BAN_CODE ;
         EXCEPTION
          WHEN OTHERS THEN
               RETURN ERROR;
         END ;
     ...
     /*  BEGIN
          ...
          INTO   P_COU_CODE
          FROM COUNTRY
          WHERE COU_IDEN = CUSTOMER.COUNTRYCODE ;
       EXCEPTION
          WHEN OTHERS THEN
              RETURN ERROR;
       END ;*/
      ...
      P_DEBIT_APPLICATION.DAP_DEPE_NUM1  :=  NULL;
       P_DEBIT_APPLICATION.DAP_PPO_COD1   :=  NULL;
     --  P_DEBIT_APPLICATION.DAP_CHLD_NAME        :=   CUSTOMER.PRINTEDNAME  ;
       P_DEBIT_APPLICATION.DAP_CHLD_CORP_NAME   :=   null ;
       ...
       P_DEBIT_APPLICATION.DAP_OLD_NEW_ACCT  := 'O'  ;
    --   P_DEBIT_APPLICATION.DAP_ATY_CODE :=1 ;
     --  P_DEBIT_APPLICATION.DAP_REQU_ACC_NUMB := CUSTOMER.ACCOUNT;
   /*
       SEARCH_CODE_LABEL('CURRENCY', 'CUR_CODE', 'CUR_IDE',SUBSTR(P_LINE,506,3), P_CUR_CODE);
       P_DEBIT_APPLICATION.DAP_REQU_ACC_CURR_CODE :=P_CUR_CODE;

       P_DEBIT_APPLICATION.DAP_REQU_EFFE_DATE := NVL(TO_DATE(TRIM(SUBSTR(P_LINE,559,8)),'RRRRMMDD'),SYSDATE);
       P_DEBIT_APPLICATION.DAP_PRIM_SECO_CARD := SUBSTR(P_LINE,567,1);*/

       -- P_DEBIT_APPLICATION.DAP_PRIM_SECO_CARD := SUBSTR(P_LINE,568,19); CTRL
    /*
        BEGIN
           SELECT  BRA_CODE
           INTO    P_DEBIT_APPLICATION.DAP_BRA_CODE
           FROM    BRANCH
              WHERE BRA_IDEN = TRIM(SUBSTR(P_LINE,597,10))
                    AND BRA_BANK_CODE= P_DEBIT_APPLICATION.DAP_BAN_CODE ;
        EXCEPTION
              WHEN OTHERS THEN
              ROLLBACK ;
               WRITE_BATCH_ERRORS(BATCH_ID,'DEBIT_APPLICATION_PKG','PREPARE_APPLICATION',
               'APP_ID:'||SUBSTR(P_LINE,11,20) ,NULL,'APP_ID:'||SUBSTR(P_LINE,11,20),SYSDATE,RESPONSE_CODE,'BRANCH_ERROR',
               SQLCODE,SQLERRM);
               COMMIT;
               RAISE  RAISE_FUNC ;
        END ;
        ...
        BEGIN
           SELECT  DPR_CODE
           INTO    P_DEBIT_APPLICATION.DAP_REQU_DPR_CODE
           FROM    DEBIT_PROGRAM
              WHERE DPR_IDEN = TRIM(SUBSTR(P_LINE,607,10))
                AND DPR_BAN_CODE= P_DEBIT_APPLICATION.DAP_BAN_CODE ;
        EXCEPTION
              WHEN OTHERS THEN
              ROLLBACK ;
               WRITE_BATCH_ERRORS(BATCH_ID,'DEBIT_APPLICATION_PKG','PREPARE_APPLICATION',
               'APP_ID:'||SUBSTR(P_LINE,11,20) ,NULL,'APP_ID:'||SUBSTR(P_LINE,11,20),SYSDATE,RESPONSE_CODE,'DEBIT_PROGRAM_ERROR',
               SQLCODE,SQLERRM);
               COMMIT;
               RAISE  RAISE_FUNC ;
        END ;
        ...
        BEGIN
           SELECT  CPR_CODE ,
                   CPR_PFE_CODE  , CPR_LIFE_CYCL ,
                   CPR_PRIM_NBI_CODE , CPR_PRIM_NBI_IDEN  ,  CPR_CMF_CODE

           INTO    P_DEBIT_APPLICATION.DAP_REQU_CPR_CODE,
                   P_DEBIT_APPLICATION.DAP_PERS_FEE_CODE , CPR_LIFE_CYCL,
                   P_NBI_CODE , P_NBI_IDEN , P_DEBIT_APPLICATION.DAP_MEMB_FEE_CODE

           FROM    CARD_PROGRAM
              WHERE CPR_IDEN = TRIM(SUBSTR(P_LINE,617, 10 ))
                AND CPR_BAN_CODE= P_DEBIT_APPLICATION.DAP_BAN_CODE ;
           P_DEBIT_APPLICATION.DAP_PAN_GENE_MANU_AUTO:='A';

           SEARCH_CODE_LABEL('NATIONAL_BIN','NBI_PAN_LENG','NBI_CODE',P_NBI_CODE,P_PAN_LENG);

          RET:=CARD_GENERATION_PKG.CARD_NUMBER_GENERATION(P_DEBIT_APPLICATION.DAP_BAN_CODE,P_DEBIT_APPLICATION.DAP_BRA_CODE,
                                                                                                        P_NBI_IDEN,
                                                                                                        P_PAN_LENG,
                                                                                                        P_DEBIT_APPLICATION.DAP_CUS_CODE,
                                                                                                        P_DEBIT_APPLICATION.DAP_REQU_CARD_NUMB,
                                                                                                        NULL,
                                                                                                        RESPONSE_CODE);

        BEGIN
           SELECT  ADD_MONTHS(NVL(P_DEBIT_APPLICATION.DAP_REQU_EFFE_DATE,SYSDATE) ,CPR_LIFE_CYCL*12)
           INTO    P_DEBIT_APPLICATION.DAP_REQU_EXPI_DATE
           FROM    DUAL;
       ...
       P_DEBIT_APPLICATION.DAP_AST_CODE:=1 ;
       P_DEBIT_APPLICATION.DAP_AST_DATE:=SYSDATE ;
       P_DEBIT_APPLICATION.DAP_PST_CODE:=1 ;
       P_DEBIT_APPLICATION.DAP_PROC_DATE:=SYSDATE ;
       P_DEBIT_APPLICATION.DAP_DATE:=SYSDATE ;
       P_DEBIT_APPLICATION.DAP_VCA_CODE := 0 ;
       */
...
-- Generated 18-Feb-21 08:51:09 from API_SIF_MOBILE@(DESCRIPTION =(ADDRESS_LIST =(ADDRESS = (PROTOCOL = TCP)(HOST = 172.18.1.199)(PORT = 1528)))(CONNECT_DATA =(SERVICE_NAME = afb12c)))
